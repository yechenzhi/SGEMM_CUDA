...
void run_bf16AB_wmma_async(int M, int N, int K, float alpha, __nv_bfloat16 *A, __nv_bfloat16 *B,
  ...
  // 告诉CUDA运行时为这个内核函数预留更多的共享内存
  // 这被称为 "opting in"
  cudaFuncSetAttribute(bf16AB_wmma_async<BM, BN, BK, WM, WN, TM, TN, TK, NUM_THREADS>,
                       cudaFuncAttributeMaxDynamicSharedMemorySize,
                       sharedMemSizeInBytes);
...
void run_bf16AB_wmma_double_buffer(int M, int N, int K, float alpha, __nv_bfloat16 *A, __nv_bfloat16 *B,
  ...
  // 告诉CUDA运行时为这个内核函数预留更多的共享内存
  // 这被称为 "opting in"
  cudaFuncSetAttribute(bf16AB_wmma_double_buffer<BM, BN, BK, WM, WN, TM, TN, TK, NUM_THREADS>,
                       cudaFuncAttributeMaxDynamicSharedMemorySize,
                       sharedMemSizeInBytes);
