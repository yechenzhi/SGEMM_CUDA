...
__global__ void __launch_bounds__(NUM_THREADS) 
    my_sgemmWarptiling_device(int M, int N, int K, float alpha,
  ...
  for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
    mywt::loadFromGmem<BM, BN, BK, stride_As, stride_Bs>(N, K, A, B, 
                    As, Bs, as_row, as_col, bs_row, bs_col);
    __syncthreads();

    mywt::processFromSmem<BM, BK, BN, WM, WN, WMITER, WNITER, TM, TN, WMSUB, WNSUB>(
