...
__global__ void bf16_wmma_cshare(int M, int N, int K, float alpha, const float *A,
...
//   wmma::fragment<wmma::accumulator, TM, TN, TK, float> acc_frag;
  ...
  for (uint k_base = 0; k_base < K; k_base += BK) {
    ...
    bf16_cshare::loadFromGmem<BM, BN, BK, stride_As, stride_Bs>(N, K, A, B, 
