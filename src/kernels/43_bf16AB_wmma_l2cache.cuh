...
__global__ void bf16AB_wmma_l2cache(int M, int N, int K, float alpha, const bf16 *A,
  ...
  //for block swizzling, can increase L2 cache hit rate
  const uint NUM_BLOCKS_M = CEIL_DIV(M, BM);
  const uint NUM_BLOCKS_N = CEIL_DIV(N, BN);
  const uint NUM_BLOCKS_IN_GROUP = GROUP_SIZE_M * NUM_BLOCKS_N;
  const uint group_id = blockIdx.x / NUM_BLOCKS_IN_GROUP;
  const uint first_bid_m = group_id * GROUP_SIZE_M; // bid_m is block id in M dimension
  uint group_size_m = NUM_BLOCKS_M - first_bid_m;
  if (group_size_m > GROUP_SIZE_M) {
      group_size_m = GROUP_SIZE_M;
  }
  const uint bid_m = first_bid_m + (blockIdx.x % NUM_BLOCKS_IN_GROUP) % group_size_m;
  const uint bid_n = (blockIdx.x % NUM_BLOCKS_IN_GROUP) / group_size_m;
  const uint row_start_block = bid_m * BM;
  const uint col_start_block = bid_n * BN;
