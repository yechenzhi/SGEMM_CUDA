...
__global__ void bf16_wmma_simple(int M, int N, int K, float alpha, const float *A,
  ...
  wmma::fragment<wmma::matrix_b, TM, TN, TK, __nv_bfloat16, wmma::col_major> b_frag;
  ...
  for (uint k_base = 0; k_base < K; k_base += BK) {
    ...
    mywt2::loadFromGmem<BM, BN, BK, stride_As, stride_Bs>(N, K, A, B, 
    ...
    for(uint k_inner = 0; k_inner < BK; k_inner += TK) {
        ...
        wmma::load_matrix_sync(b_frag, &Bs[bRow + bCol * ldb], ldb);
  ...
  const uint ldc = N;
  const uint c_row = row_start_block + warp_row * TM;
  const uint c_col = col_start_block + warp_col * TN;
  if ( c_row < M && c_col < N ) {
