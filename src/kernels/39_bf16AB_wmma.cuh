...
namespace bf16AB_warptiling{
    template <const int BM, const int BN, const int BK, const int stride_As, const int stride_Bs>
    __device__ void loadFromGmem(int N, int K, const bf16 *A, const bf16 *B,
                                 bf16 *As, bf16 *Bs,
        ...
        for(uint row_offset = 0; row_offset < BM; row_offset += stride_As) {
            ...
            reinterpret_cast<int4*>(dst_ptr)[0] = reinterpret_cast<const int4*>(src_ptr)[0];
        }
...
__global__ void bf16AB_wmma_warptiling(int M, int N, int K, float alpha, const bf16 *A,
...
//   wmma::fragment<wmma::accumulator, TM, TN, TK, float> acc_frag;
  ...
  for (uint k_base = 0; k_base < K; k_base += BK) {
    ...
    bf16AB_warptiling::loadFromGmem<BM, BN, BK, stride_As, stride_Bs>(N, K, A, B, 
